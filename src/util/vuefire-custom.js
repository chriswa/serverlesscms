...
function bind (vm, key, source) {
  ...
  if (asObject) {
    bindAsObject(vm, key, source, cancelCallback);
  	...
  	bindAsScalar(vm, key, source, cancelCallback);
  }
	else {
...
function bindAsScalar (vm, key, source, cancelCallback) {
...
var init = function () {
  ...
  // new feature
	...
	//console.log(".firebaseAllLoaded = false")
  defineReactive(this$1, ".firebaseAllLoaded", false);

	var oncePromises = []
	for (var key in this._firebaseSources) {
		oncePromises.push( this._firebaseSources[ key ].once('value') )
	}
	Promise.all( oncePromises ).then( () => {
		//console.log(".firebaseAllLoaded = true")
		defineReactive(this$1, ".firebaseAllLoaded", true);
	})
};
